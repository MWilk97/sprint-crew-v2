...
[`api/console/`](src/sprint_crew/api/console/) is layered so the import graph stays acyclic: `state` (router, per-session locks, the persistence seam) ← `clarify` (Interpreter + answer validation) and `run_bridge` (console↔backlog-run translation) ← `routes` (session lifecycle), `events` (polling + SSE), `plan` (plan-mode runs + promote), `ask` (codebase chat), `diffs` (snapshots + review decisions) and `attachments` (uploads) ← `routes`, which resolves attachment ids for a message. Importing the package registers every route.
